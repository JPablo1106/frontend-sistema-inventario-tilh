...
const EquiposSeguridad = () => {
  const [asignacionesEquiposSeguridad, setAsignacionesEquiposSeguridad] = useState([])
  ...
  useEffect(() => {
    fetchAsignacionesEquiposSeguridad()
  }, [])

  const fetchAsignacionesEquiposSeguridad = async () => {
    setIsLoading(true)
    setError(false)
    ...
    try {
      ...
      const asignacionesEquiposSeguridadArray = Array.isArray(response.data)
        ? response.data
        : Object.values(response.data)
      setAsignacionesEquiposSeguridad(asignacionesEquiposSeguridadArray)

      const ahora = new Date()
      setUltimaActualizacion(`${ahora.toLocaleDateString("es-ES")} ${ahora.toLocaleTimeString("es-ES")}`)
    } catch (error) {
      console.error("Error al obtener asignaciones de equipos de seguridad:", error)
      setError(true)
      Swal.fire({
        title: "Error de conexión",
        text: "No se pudieron cargar las asignaciones de equipos de seguridad. Intente nuevamente.",
        icon: "error",
  ...
  const handleAgregarEquipoSeguridad = useCallback(() => {
    navigate("/equipos-seguridad/registro-equipo-seguridad")
  }, [navigate])

  // Filtrar datos según término de búsqueda
  const filtracionDatos = useMemo(() => {
    if (!busquedaFilt.trim()) return asignacionesEquiposSeguridad
    return asignacionesEquiposSeguridad.filter(
      (equipoSeguridad) =>
        equipoSeguridad.usuario?.nombreUsuario.toLowerCase().includes(busquedaFilt.toLowerCase()) ||
        equipoSeguridad.usuario?.area?.toLowerCase().includes(busquedaFilt.toLowerCase()) ||
        equipoSeguridad.usuario?.departamento?.toLowerCase().includes(busquedaFilt.toLowerCase()) ||
        equipoSeguridad.idAsignacion.toString().includes(busquedaFilt),
    )
  }, [asignacionesEquiposSeguridad, busquedaFilt])
  ...
  const handleExportExcel = async () => {
    ...
    try {
      ...
      const exportData = filtracionDatos.map((item) => {
        // Formateo de fecha
        let fechaAsignacion = "Sin datos"
        if (item.fechaAsignacion) {
          const date = new Date(item.fechaAsignacion)
          fechaAsignacion = `${String(date.getDate()).padStart(2, "0")}-${String(date.getMonth() + 1).padStart(
            2,
            "0",
          )}-${date.getFullYear()}`
        }
        const detalle = (item.detalleAsignaciones && item.detalleAsignaciones[0]) || {}
        const equipo = detalle.equipoSeguridad || {}
        return {
          ...
          Tipo: equipo.tipo || "Sin datos",
          Marca: equipo.marca || "Sin datos",
          Modelo: equipo.modelo || "Sin datos",
          Capacidad: equipo.capacidad || "Sin datos",
          "Número de Serie": detalle.numSerieEquipoSeg || "Sin datos",
        }
      })

      // Crear workbook y worksheet con ExcelJS
      const workbook = new ExcelJS.Workbook()
      const worksheet = workbook.addWorksheet("Equipos Seguridad")
      ...
      saveAs(blob, "EquiposSeguridad_Asignados.xlsx")
          ...
          <div className="col-md-6 text-center">
            <div className="card shadow-lg border-0">
              <div className="card-body p-5">
                ...
                <h4 className="mb-3">Cargando datos de equipos de seguridad</h4>
          ...
          <div className="col-md-6 text-center">
            <div className="card shadow-lg border-0 bg-light">
              <div className="card-body p-5">
                ...
                <p className="text-muted mb-4">No se pudieron cargar los datos de equipos de seguridad asignados.</p>
                <button className="btn btn-primary" onClick={fetchAsignacionesEquiposSeguridad}>
    ...
    <div className="container-fluid mt-4">
      <div className="row mb-4 align-items-center">
        <div className="col-md-6">
          ...
          <p className="text-muted">Administración de equipos de seguridad asignados en el sistema de inventario</p>
        </div>
        <div className="col-md-6 text-md-end">
          <button className="btn btn-outline-primary me-2" onClick={fetchAsignacionesEquiposSeguridad}>
          ...
          <div className="card shadow-sm border-0">
            <div className="card-header bg-white d-flex justify-content-between align-items-center py-3">
              <div className="d-flex align-items-center">
                <FaShieldAlt className="text-warning me-2" style={{ fontSize: "1.2rem" }} />
                <h5 className="mb-0">Listado de Equipos de Seguridad Asignados</h5>
              </div>
              <div className="d-flex gap-2">
                ...
                <button
                  ...
                  onClick={handleAgregarEquipoSeguridad}
                  title="Agregar nuevo equipo de seguridad"
                >
                  <FaPlus className="me-1" /> Nuevo Equipo de Seguridad
                </button>
              </div>
            </div>
                      ...
                      <tr>
                        <td colSpan={columns.length} className="text-center py-4">
                          <div className="alert alert-info mb-0">
                            ...
                            No hay equipos de seguridad asignados para mostrar
                          </div>
                        </td>
                      </tr>
                    )}
      ...
      <div className="row mt-2">
        <div className="col-12">
          <div className="alert alert-info d-flex align-items-center" role="alert">
            <FaInfoCircle className="me-2" />
            <div>
              <strong>Información:</strong> Esta tabla muestra los equipos de seguridad asignados a usuarios en el
              sistema. Para agregar un nuevo equipo de seguridad, haga clic en "Nuevo Equipo de Seguridad".
            </div>
          </div>
        </div>
      </div>
...
export default EquiposSeguridad
