...
import { FaSearch, FaPlus, FaFileExcel, FaSync, FaInfoCircle, FaPhone } from "react-icons/fa"
...
const ComponentesTelefono = () => {
  const [asignacionesTelefonos, setAsignacionesTelefonos] = useState([])
  ...
  useEffect(() => {
    fetchAsignacionesTelefonos()
  }, [])

  const fetchAsignacionesTelefonos = async () => {
    setIsLoading(true)
    setError(false)
    ...
    try {
      ...
      const response = await axios.get(
        "https://backendsistemainventario.onrender.com/api/Asignaciones/ConsultarAsignaciones",
        {
          ...
          params: { tipo: "telefonos" },
        },
      )

      const asignacionesTelefonosArray = Array.isArray(response.data) ? response.data : Object.values(response.data)
      setAsignacionesTelefonos(asignacionesTelefonosArray)

      const ahora = new Date()
      setUltimaActualizacion(`${ahora.toLocaleDateString("es-ES")} ${ahora.toLocaleTimeString("es-ES")}`)
    } catch (error) {
      console.error("Error al obtener asignaciones de teléfonos:", error)
      setError(true)
      Swal.fire({
        title: "Error de conexión",
        text: "No se pudieron cargar las asignaciones de teléfonos. Intente nuevamente.",
        icon: "error",
  ...
  const handleAgregarTelefono = useCallback(() => {
    navigate("/componentes/registro-componente")
  }, [navigate])

  // Filtrar datos según término de búsqueda
  const filtracionDatos = useMemo(() => {
    if (!busquedaFilt.trim()) return asignacionesTelefonos
    return asignacionesTelefonos.filter(
      (telefono) =>
        telefono.usuario?.nombreUsuario.toLowerCase().includes(busquedaFilt.toLowerCase()) ||
        telefono.usuario?.area?.toLowerCase().includes(busquedaFilt.toLowerCase()) ||
        telefono.usuario?.departamento?.toLowerCase().includes(busquedaFilt.toLowerCase()) ||
        telefono.idAsignacion.toString().includes(busquedaFilt),
    )
  }, [asignacionesTelefonos, busquedaFilt])
    ...
    try {
      ...
      const exportData = filtracionDatos.map((item) => {
        ...
        return {
          ...
          Modelo: detalle.componente?.modeloTelefono || "Sin datos",
      ...
      const worksheet = workbook.addWorksheet("Teléfonos Asignados")
      ...
      saveAs(blob, "Telefonos_Asignados.xlsx")
    ...
    () =>
      [
        ...
        },
        {
          header: "Modelo",
          cell: ({ row }) =>
            (row.original.detalleAsignaciones && row.original.detalleAsignaciones[0]?.componente?.modeloTelefono) ||
            "Sin datos",
        },
        {
          ...
          <div className="col-md-6 text-center">
            <div className="card shadow-lg border-0">
              <div className="card-body p-5">
                ...
                <h4 className="mb-3">Cargando datos de teléfonos</h4>
          ...
          <div className="col-md-6 text-center">
            <div className="card shadow-lg border-0 bg-light">
              <div className="card-body p-5">
                ...
                <p className="text-muted mb-4">No se pudieron cargar los datos de teléfonos asignados.</p>
                <button className="btn btn-primary" onClick={fetchAsignacionesTelefonos}>
    ...
    <div className="container-fluid mt-4">
      <div className="row mb-4 align-items-center">
        <div className="col-md-6">
          ...
          <p className="text-muted">Administración de teléfonos asignados en el sistema de inventario</p>
        </div>
        <div className="col-md-6 text-md-end">
          <button className="btn btn-outline-primary me-2" onClick={fetchAsignacionesTelefonos}>
          ...
          <div className="card shadow-sm border-0">
            <div className="card-header bg-white d-flex justify-content-between align-items-center py-3">
              <div className="d-flex align-items-center">
                <FaPhone className="text-danger me-2" style={{ fontSize: "1.2rem" }} />
                <h5 className="mb-0">Listado de Teléfonos Asignados</h5>
              </div>
              <div className="d-flex gap-2">
                ...
                >
                  <FaPlus className="me-1" /> Nuevo Teléfono
                </button>
              </div>
            </div>
                      ...
                      <tr>
                        <td colSpan={columns.length} className="text-center py-4">
                          <div className="alert alert-info mb-0">
                            ...
                            No hay teléfonos asignados para mostrar
                          </div>
                        </td>
                      </tr>
                    )}
      ...
      <div className="row mt-2">
        <div className="col-12">
          <div className="alert alert-info d-flex align-items-center" role="alert">
            <FaInfoCircle className="me-2" />
            <div>
              <strong>Información:</strong> Esta tabla muestra los teléfonos asignados a usuarios en el sistema. Para
              agregar un nuevo teléfono, haga clic en "Nuevo Teléfono".
            </div>
          </div>
        </div>
      </div>
...
export default ComponentesTelefono