...
import { FaSearch, FaPlus, FaFileExcel, FaSync, FaInfoCircle, FaDesktop, FaTv } from "react-icons/fa"
...
const ComponentesMonitores = () => {
  const [asignacionesMonitores, setAsignacionesMonitores] = useState([])
  ...
  useEffect(() => {
    fetchAsignacionesMonitores()
  }, [])

  const fetchAsignacionesMonitores = async () => {
    setIsLoading(true)
    setError(false)
    ...
    try {
      ...
      const response = await api.get(
        "Asignaciones/ConsultarAsignaciones",
        {
          ...
          params: { tipo: "monitores" },
        },
      )

      const asignacionesMonitoresArray = Array.isArray(response.data) ? response.data : Object.values(response.data)
      setAsignacionesMonitores(asignacionesMonitoresArray)

      const ahora = new Date()
      setUltimaActualizacion(`${ahora.toLocaleDateString("es-ES")} ${ahora.toLocaleTimeString("es-ES")}`)
    } catch (error) {
      console.error("Error al obtener asignaciones de monitores:", error)
      setError(true)
      Swal.fire({
        title: "Error de conexión",
        text: "No se pudieron cargar las asignaciones de monitores. Intente nuevamente.",
        icon: "error",
  ...
  const handleAgregarMonitor = useCallback(() => {
    navigate("/componentes/registro-componente", { replace: true })
  }, [navigate])

  // Filtrar datos según término de búsqueda
  const filtracionDatos = useMemo(() => {
    if (!busquedaFilt.trim()) return asignacionesMonitores
    return asignacionesMonitores.filter(
      (monitor) =>
        monitor.usuario?.nombreUsuario.toLowerCase().includes(busquedaFilt.toLowerCase()) ||
        monitor.usuario?.area?.toLowerCase().includes(busquedaFilt.toLowerCase()) ||
        monitor.usuario?.departamento?.toLowerCase().includes(busquedaFilt.toLowerCase()) ||
        monitor.idAsignacion.toString().includes(busquedaFilt),
    )
  }, [asignacionesMonitores, busquedaFilt])
    ...
    try {
      ...
      const exportData = filtracionDatos.map((item) => {
        ...
        return {
          ...
          Modelo: detalle.componente?.modeloMonitor || "Sin datos",
      ...
      const worksheet = workbook.addWorksheet("Monitores Asignados")
      ...
      saveAs(blob, "Monitores_Asignados.xlsx")
    ...
    () =>
      [
        ...
        },
        {
          header: "Modelo",
          cell: ({ row }) =>
            (row.original.detalleAsignaciones && row.original.detalleAsignaciones[0]?.componente?.modeloMonitor) ||
            "Sin datos",
        },
        {
          ...
          <div className="col-md-6 text-center">
            <div className="card shadow-lg border-0">
              <div className="card-body p-5">
                ...
                <h4 className="mb-3">Cargando datos de monitores</h4>
          ...
          <div className="col-md-6 text-center">
            <div className="card shadow-lg border-0 bg-light">
              <div className="card-body p-5">
                ...
                <p className="text-muted mb-4">No se pudieron cargar los datos de monitores asignados.</p>
                <button className="btn btn-primary" onClick={fetchAsignacionesMonitores}>
    ...
    <div className="container-fluid mt-4">
      <div className="row mb-4 align-items-center">
        <div className="col-md-6">
          ...
          <p className="text-muted">Administración de monitores asignados en el sistema de inventario</p>
        </div>
        <div className="col-md-6 text-md-end">
          <button className="btn btn-outline-primary me-2" onClick={fetchAsignacionesMonitores}>
          ...
          <div className="card shadow-sm border-0">
            <div className="card-header bg-white d-flex justify-content-between align-items-center py-3">
              <div className="d-flex align-items-center">
                <FaDesktop className="text-info me-2" style={{ fontSize: "1.2rem" }} />
                <h5 className="mb-0">Listado de Monitores Asignados</h5>
              </div>
              <div className="d-flex gap-2">
                ...
                <button className="btn btn-primary btn-sm" onClick={handleAgregarMonitor} title="Agregar nuevo monitor">
                  <FaPlus className="me-1" /> Nuevo Monitor
                </button>
              </div>
            </div>
                      ...
                      <tr>
                        <td colSpan={columns.length} className="text-center py-4">
                          <div className="alert alert-info mb-0">
                            ...
                            No hay monitores asignados para mostrar
                          </div>
                        </td>
                      </tr>
                    )}
      ...
      <div className="row mt-2">
        <div className="col-12">
          <div className="alert alert-info d-flex align-items-center" role="alert">
            <FaInfoCircle className="me-2" />
            <div>
              <strong>Información:</strong> Esta tabla muestra los monitores asignados a usuarios en el sistema. Para
              agregar un nuevo monitor, haga clic en "Nuevo Monitor".
            </div>
          </div>
        </div>
      </div>
...
export default ComponentesMonitores
